...
| 4.4 status two-source merge | ✅ done (2026-07-11) | Adapter `GET .../target-status` serves per-declaration ledger evidence (file existence, sorry scan — Python twin of `containsSorryMarker` — and the newest check verdict across ALL the project's sessions). Companion `getTheoremStatusLedger` behind `LEA_STATUS_ENGINE=ledger` (legacy default this release): job overlay decides lifecycle, ledger decides file truth; no markdown parsing, no FS regex probes. Harness runs both engines on one live stack and asserts agreement on the scripted scenarios. Known open semantic: after a failed retry restores the previous proof, legacy reports the failed run, ledger the restored file's validity — settle before flipping the default. |
| 4.3 registry demotion | ✅ done to the safe boundary (2026-07-11) | Adapter backfills the artifacts table from a pre-index project's registry markdown (one-time, guarded by "no rows yet"; backfilled rows carry NULL session/run so they can never masquerade as run-recorded). With the index comprehensive and the ledger engine markdown-free, the registry is write-only for machines on the ledger path. Remaining (explicitly gated on the 4.2 bake + ledger default flip): delete the legacy engine's markdown readers, the identification fallback, and switch `uses=` resolution to the index. |
