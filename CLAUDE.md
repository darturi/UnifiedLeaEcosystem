...
- **Shared process state** (`LEA_SHARED_STATE=true`): the Overleaf companion records
  each run's full timeline into the *same* SQLite DB the UI uses, so Overleaf runs
  appear in the UI and can be deep-linked. Overrides: `LEA_SHARED_DATA_DIR`,
  `LEA_DB_PATH`, `LEA_EVENT_LOG_DIR`. Design: `docs/shared-process-state.md`.
...
docs/                 shared-process-state.md (design), migration & planning notes
```
